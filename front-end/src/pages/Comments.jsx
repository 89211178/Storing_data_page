...
function Comments() {
  ...
  const recipe_title = searchParams.get("recipe_title"); // Extract the recipe title from the URL
  ...
  const handleSubmit = async (event) => {
    ...
    const values = {
      ...
      rating,
      comment,
      recipe_title, // Include the recipe title in the values to be saved in the database
    };

    try {
      const response = await axios.post("http://88.200.63.148:3082/Comments", values);
...
function Comments() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post("http://88.200.63.148:3080/Comments", values);
      if (response.status === 200) {
        navigate("/Home");
      } else {
        alert("Failed to submit the comment. Please try again.");
